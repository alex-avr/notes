...
fun ICorChainDsl<FolderContext>.folderStubs(title: String, block: ICorChainDsl<FolderContext>.() -> Unit) = chain {
    block()
    this.title = title
    on { workMode == NotesWorkMode.STUB && state == NotesState.RUNNING }
}

fun ICorChainDsl<NoteContext>.noteStubs(title: String, block: ICorChainDsl<NoteContext>.() -> Unit) = chain {
    block()
    this.title = title
    on { workMode == NotesWorkMode.STUB && state == NotesState.RUNNING }
}