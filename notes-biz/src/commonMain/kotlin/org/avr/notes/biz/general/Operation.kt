...
fun ICorChainDsl<FolderContext>.folderOperation(title: String, command: FolderCommand, block: ICorChainDsl<FolderContext>.() -> Unit) = chain {
    block()
    this.title = title
    on { this.command == command && state == NotesState.RUNNING }
}

fun ICorChainDsl<NoteContext>.noteOperation(title: String, command: NoteCommand, block: ICorChainDsl<NoteContext>.() -> Unit) = chain {
    block()
    this.title = title
    on { this.command == command && state == NotesState.RUNNING }
}